...
def save_bgr_mp4_atomic(frames, output_file, fps):
    """
    """
    if len(frames) == 0:
        return False
    ...
    container = None
    try:
        container = av.open(str(tmp_file), mode="w", format="mp4")
        ...
        if container is not None:
            container.close()
        if tmp_file.exists() and not output_file.exists():
            try:
                tmp_file.unlink()
            except OSError:
                pass
...
class Stage3SimulationRunner(SimulationRunner):
    ...
    def __init__(
        ...
        self.traj_straight_plan_steps = 6
    ...
    def run(self) -> RunnerReport:
        ...
        refresh_every_frames = int(os.environ.get("VIDEO_REFRESH_EVERY_FRAMES", "5"))
        last_saved_video_len = 0
        save_proj_preview = False
        ...
        print(f"[VIDEO] single output = {video_file}")
        print(f"[VIDEO] refresh every {refresh_every_frames} generated frames")
        ...
        while self.simulation.is_simulation_running():
            ...
            if max_steps is not None and global_count >= max_steps:
            ...
            if global_count % plan_every_10hz_steps == 0:
                ...
                traj_states = [scenario.get_ego_state_at_iteration(iter_idx)]
                ...
                if len(future_states) == 0:
                    try:
                        scenario_num_iters = int(scenario.get_number_of_iterations())
                        last_iter_idx = max(0, scenario_num_iters - 1)

                        for k in range(1, 23):
                            j = iter_idx + k
                            if j > last_iter_idx:
                                break
                            traj_states.append(scenario.get_ego_state_at_iteration(j))
                    except Exception:
                        pass
                else:
                    traj_states.extend(future_states)

                if len(traj_states) < 2:
                    print(f"[DB_TRAJ] no future state at iter={iter_idx}, stop before propagate")
                    break

                trajectory = InterpolatedTrajectory(trajectory=traj_states)
                trajectory_source = "NuPlanDB"
            ...
            try:
                ...
                if global_count % plan_every_10hz_steps == 0:
                    ...
                    if save_proj_preview:
                        self.proj_triplet_frames.append(None)

                    if save_proj_preview and global_count >= warmup_steps and cond_pack is not None and "proj_sem" in cond_pack and "proj_clr" in cond_pack:
                        t_cur = self.rollbuf.history_len
                    ...
                    if global_count == warmup_steps and len(self.overlay_hist_frames) == self.rollbuf.history_len:
                        ...
                        if save_proj_preview and cond_pack is not None and "proj_sem" in cond_pack and "proj_clr" in cond_pack:
                    ...
                    if global_count >= warmup_steps:
                        new_video_frames = len(video_frames) - last_saved_video_len
                        if new_video_frames >= refresh_every_frames:
                            ok = save_bgr_mp4_atomic(
                                frames=video_frames,
                            ...
                            if ok:
                                last_saved_video_len = len(video_frames)
                                print(
                                    f"✓ refreshed video: {video_file} "
        ...
        if len(video_frames) > 0 and len(video_frames) != last_saved_video_len:
            ok = save_bgr_mp4_atomic(
                frames=video_frames,
                output_file=video_file,
                fps=self.video_fps,
            ...
            if ok:
                last_saved_video_len = len(video_frames)
                print(f"✓ final refreshed video: {video_file} frames={len(video_frames)}")
