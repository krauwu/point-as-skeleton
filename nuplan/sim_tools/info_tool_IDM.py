...
def build_future_info_from_state(
    template_info,
    ego_state,
    future_dt_s,
    k,
    tracked_objects_future=None,
    db_record=None,
    trans_z=None,
):
    """
    ...
    try:
        ...
        if info.get("timestamp", None) is not None:
            info["timestamp"] = int(info["timestamp"] + (k + 1) * future_dt_s * 1e6)
    ...
    if trans_z is None:
        z = 0.0
        if template_info.get("egopose", None) is not None and len(template_info["egopose"]) >= 3:
            z = float(template_info["egopose"][2])
    else:
        z = float(trans_z)

    info["egopose"] = [x, y, z, yaw]
    ...
    # cam
    # -------------------------
    if db_record is not None and ego2global is not None:
        info["cam"] = build_cam_dict_from_db_record(db_record, ego2global)
    ...
    info["gt_names"] = np.zeros((0,), dtype=object)
    info["track_token"] = np.zeros((0,), dtype=object)

    info["tracked_objects"] = tracked_objects_future
...
class RollingInfoBuffer5Hz:
    ...
    def update_generated_frame(self, timestamp, img_filenames_8, img_tokens_8):
        """
        """
        self.generated_cache[int(timestamp)] = {
            "img_filename": img_filenames_8,
            "img_tokens": img_tokens_8,
        }
    ...
    def build_infos(self, planner_input, future_trajectory, past_ego_states=None):
        ...
        obs_buf = hist.observation_buffer
        ...
        if len(past_infos) == 0:
            assert len(past_infos) > 0, (
                ...
                f"len(ego_buf)={len(ego_buf)} len(obs_buf)={len(obs_buf)} "
                f"stride_sim={self.stride_sim} history_len={self.history_len}"
            )

        future_infos = []
        tracked_now = None
        obs_now = obs_buf[-1] if len(obs_buf) > 0 else None
        if obs_now is not None:
            tracked_now = obs_now.tracked_objects
        ...
        if future_trajectory is not None:
            ...
            for k, st in enumerate(needed):
                info_f = build_future_info_from_state(
                    template_info=template_info,
                    ego_state=st,
                    future_dt_s=future_dt_s,
                    k=k,
                    tracked_objects_future=tracked_now,
                    db_record=self.db_record,
                    trans_z=self.trans_z,
                )
                future_infos.append(info_f)

        infos_all = past_infos + future_infos

        infos_all = fill_future_boxes_rollout_reactive_simple(
            infos_all,
            history_len=self.history_len,
            pack_hz=self.pack_hz,
        )
