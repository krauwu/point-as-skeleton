...
def stack_conds(ds):
    ...
    for k in ds[0].keys():
        v0 = ds[0][k]
        if v0 is None or (not torch.is_tensor(v0)):
            out[k] = v0
        else:
            out[k] = torch.cat([d[k] for d in ds], dim=0)
    return out

def pick_half(v, cond: bool):
    if v is None or (not torch.is_tensor(v)):
        return v
    u, c = v.chunk(2, dim=0)
    return c if cond else u

def install_sanitize_hooks(root_module, patterns=("ff_context.net.2",), lo=-1e4, hi=1e4,
                            verbose_rank0=True):
    def _fix(x, name):
        ...
        mean = fin.mean() if fin.numel() else torch.tensor(0., device=x.device, dtype=x.dtype)
        y = torch.nan_to_num(x, nan=float(mean), posinf=hi, neginf=lo).clamp(lo, hi)
        return y
    ...
    def _hook(name):
        def h(_m, _inp, out):
            ...
            if torch.is_tensor(out):
                return _fix(out, name)
            if isinstance(out, (list, tuple)):
                out2 = [_fix(o, f"{name}[{i}]") for i, o in enumerate(out)]
                return type(out)(out2)
    ...
    if verbose_rank0:
        ...
        if torch.distributed.is_available() and torch.distributed.is_initialized():
            r0 = (torch.distributed.get_rank() == 0)
        if r0:
            print(f"[SAN] hooks installed: {len(matched)})")
            for n in matched[:20]:
                print("  -", n)
            if len(matched) > 20:
                print("  ...")
...
def build_param_groups(model, lr_base=9e-6, lr_adp=3e-4, wd=0.01):
    def is_adp(n: str) -> bool:
        return ("condition_depth_adapter" in n) or ("condition_clr_adapter" in n)

    base_d, base_nd, adp_d, adp_nd = [], [], [], []
    for n, p in model.named_parameters():
        ...
        if is_adp(n):
            (adp_nd if no_decay else adp_d).append(p)
        else:
            (base_nd if no_decay else base_d).append(p)

    return [
        {"params": adp_d,   "lr": lr_adp,  "weight_decay": wd},
        {"params": adp_nd,  "lr": lr_adp,  "weight_decay": 0.0},
        {"params": base_d,  "lr": lr_base, "weight_decay": wd},
        {"params": base_nd, "lr": lr_base, "weight_decay": 0.0},
    ]
...
class CrossviewTemporalSD():
    ...
    def get_conditions(
        ...
        streaming_mode: bool = False, prev_ego_transforms=None,
        do_classifier_free_guidance: bool = False,
        latents_shape=None, depth_condition_mask=None,
        clr_proj_condition_mask=None,
    ):
        ...
        #----depth--------
        depth_img = None
        if "proj_depth" in batch:
            bin_id = batch["proj_depth"].to(device) if condition_on_all_frames \
                else batch["proj_depth"][:, :1].to(device)             # [B,T,V,H,W] or [B,1,V,H,W]
            bin_id = bin_id.long()
            valid = (bin_id != 0)

            n_bins = int(common_config["pc_cfg"].get("depth_bins", 256))
            mode  = common_config.get("proj_depth_mode", "continuous")   # "continuous"|"discrete"|"both"
            d_dim = int(common_config.get("proj_depth_embed_dim", 16))
            half  = d_dim // 2
            assert d_dim % 2 == 0, "proj_depth_embed_dim should be an even number."

            # --- continuous u: [B,T,V,1,H,W] (valid:[0,1], invalid:-1) ---
            u = bin_id.clamp(0, n_bins - 1).float() / max(1, n_bins - 1)
            u = u.masked_fill(~valid, -1.0).unsqueeze(3).to(dtype=dtype)

            # --- sincos emb: [B,T,V,d_dim,H,W] (invalid:0) ---
            id01 = (bin_id.clamp(0, n_bins - 1).float() / max(1, n_bins - 1)).masked_fill(~valid, 0.0)

            max_freq = max(1, n_bins // 2)
            freqs = 2.0 ** torch.linspace(
                0, math.log2(float(max_freq)), steps=half, device=device, dtype=dtype
            )                                                         # [half], last ~= max_freq
            ang = id01.unsqueeze(-1) * (2 * math.pi) * freqs          # [B,T,V,H,W,half]
            emb = torch.cat([torch.sin(ang), torch.cos(ang)], dim=-1) # [B,T,V,H,W,d_dim]
            emb = emb * valid.unsqueeze(-1)                           # invalid -> 0
            emb = emb.permute(0, 1, 2, 5, 3, 4).contiguous()          # [B,T,V,d_dim,H,W]

            if mode == "continuous":
                depth_img = torch.cat([u, torch.zeros_like(emb)], dim=3)
            elif mode == "discrete":
                depth_img = torch.cat([torch.full_like(u, -1.0), emb], dim=3)
            else:  # "both"
                depth_img = torch.cat([u, emb], dim=3)

            if depth_condition_mask is not None:
                m = depth_condition_mask.view(-1, 1, 1, 1, 1, 1)
                depth_img[:, :, :, :1] = torch.where(m, depth_img[:, :, :, :1], torch.full_like(depth_img[:, :, :, :1], -1.0))
                depth_img[:, :, :, 1:] = torch.where(m, depth_img[:, :, :, 1:], torch.zeros_like(depth_img[:, :, :, 1:]))

            # CFG：uncond = continuous:-1, emb:0
            if do_classifier_free_guidance:
                d_u = depth_img.clone()
                d_u[..., :1, :, :] = -1.0
                d_u[..., 1:, :, :] = 0.0
                depth_img = torch.cat([d_u, depth_img], dim=0)
        ...
        if "proj_clr" in batch:
            clr = batch["proj_clr"].to(device)
            if not condition_on_all_frames:
                clr = clr[:, :1]
 
            if clr_proj_condition_mask is not None:
                clr[clr_proj_condition_mask.logical_not().to(device)] = uncondition_image_color
                
            clr_img = clr
            if do_classifier_free_guidance:
                c_u = torch.ones_like(clr_img) * uncondition_image_color
                clr_img = torch.cat([c_u, clr_img], dim=0)
        ...
        result = {
            ...
            "condition_depth_tensor": depth_img,
            "condition_clr_tensor": clr_img
    ...
    def __init__(
        ...
        if torch.distributed.is_initialized():
            ...
            self._san_handles = install_sanitize_hooks(
                self.model_wrapper,
                patterns=("ff_context.net.2",),  
                lo=-1e4, hi=1e4,
                verbose_rank0=True
            )
        ...
        if "group_optimizer" in config:
            param_groups = build_param_groups(self.model, lr_base=config["group_optimizer"]["lr_base"],
                                          lr_adp=config["group_optimizer"]["lr_adp"], wd=0.01)
    ...
    def train_step(self, batch: dict, global_step: int):
        ...
        # ===== PC(depth/clr) sampling (keep text/layout branches unchanged) =====
        p_depth_mask = float(self.training_config.get("depth_mask_ratio", 0.1))
        p_clr_mask   = float(self.training_config.get("clr_mask_ratio",   0.1))

        depth_condition_mask = (torch.rand((batch_size,), generator=self.generator).to(self.device)  >= p_depth_mask)
        clr_proj_condition_mask = (torch.rand((batch_size,), generator=self.generator).to(self.device) >= p_clr_mask)
        # ===== end =====
        ...
        ## new depth ###
        self.common_config['pc_cfg'] = self.config['training_dataset']['base_dataset']['datasets'][0]['projected_pc_settings']
        ## new depth ###
        ...
        with self.get_autocast_context():
            model_conditions = CrossviewTemporalSD.get_conditions(
                ...
                latents_shape=latents.shape, depth_condition_mask=depth_condition_mask,
                clr_proj_condition_mask=clr_proj_condition_mask,)
    ...
    def inference_pipeline(
        ...
        ## new depth ###
        self.common_config['pc_cfg'] = self.config['validation_dataset']['base_dataset']['datasets'][0]['projected_pc_settings']
        ## new depth ###
        ...
        if do_classifier_free_guidance:
            for k in ("condition_depth_tensor", "condition_clr_tensor"):
                v = model_conditions.get(k, None)
                if v is not None:
                    _, c = v.chunk(2, dim=0)
                    model_conditions[k] = torch.cat([c, c], dim=0)
        ...
        for i in range(start_timestep, stop_timestep):
            ...
            if do_classifier_free_guidance:
                ...
                latent_in = latent_model_input
                ts_in     = timesteps

            model_output, _, _ = self.model_wrapper(latent_in, ts_in, **model_conditions)
    ...
    def autoregressive_inference_pipeline(
        ...
        if diffusion_forcing_mode:
            ...
            if flush_steps == 1:
                # testcase-only support bs=1
                for j in range(1,image_latents.shape[1]):
                    image_tensor = self.image_processor.postprocess(self.vae.decode(
                        image_latents[0][j] / self.vae.config.scaling_factor + shift_factor,
                        return_dict=False)[0], output_type=output_type)
                    result["images"].append(image_tensor)
            else:    
                for j in range(1, flush_steps):
                    frozen_i = min(dataset_ref_left + j, latent_shape[1])
                    # print('flushing!!')
                    # test_fn(latent_shape[1],
                    #     self.inference_config["inference_steps"] // (latent_shape[1] - clear_reference_frame_count),
                    #     self.inference_config["inference_steps"],
                    #     clear_reference_frame_count,
                    #     self.inference_config["inference_steps"] + (2 * j - 1 ) * steps_per_inference,
                    #     self.inference_config["inference_steps"] + (2 * j ) * steps_per_inference,
                    #     j,
                    #     dataset_ref_left)
                    ...
                    iteration_output = self.inference_pipeline(
                        ...
                        start_timestep=(
                            self.inference_config["inference_steps"] +
                            (2 * j - 1 ) * steps_per_inference
                        ),
                        stop_timestep=(
                            self.inference_config["inference_steps"] +
                            (2 * j ) * steps_per_inference
                        ),
                        take_time=j,
    ...
    def preview_pipeline(
        ...
        if self.should_save or (
            ...
            preview_tensor = dwm.utils.preview.make_ctsd_preview_tensor_test(
            ...
            if sequence_length == 1:
                ...
                dwm.utils.preview.save_tensor_to_video(
                    ...
                    pix_fmt="yuv420p",
                    stream_options={"crf": "12", "preset": "slow", "tune": "film"})
                if random.random() < 0.05:
                    preview_tensor_full = dwm.utils.preview.make_ctsd_preview_tensor(
                        pipeline_output["images"], batch, self.inference_config
                    )
                    os.makedirs(os.path.join(output_path, "preview_full"), exist_ok=True)
                    video_output_path_full = os.path.join(output_path, "preview_full", f"{filename}_full.mp4")
                    dwm.utils.preview.save_tensor_to_video(
                        video_output_path_full, "libx264", batch["fps"][0].item(), preview_tensor_full
                    )

                tok_list = [t[0] if isinstance(t, tuple) else t for t in batch['segment_samples']]
                txt_path = os.path.join(output_path, "preview_sampletok", f"{filename}.txt")
