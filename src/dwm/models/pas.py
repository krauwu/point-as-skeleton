...
def _merge_res(base, add, scale: float):
    if add is None or scale == 0:
        return base
    if base is None:
        return [r * scale for r in add]
    n = min(len(base), len(add))
    for i in range(n):
        base[i] = base[i] + add[i] * scale
    return base
...
class DiTCrossviewTemporalConditionModel(diffusers.SD3Transformer2DModel):
    ...
    def __init__(
        ...
        mask_module=None,
        condition_pc_adapter_config: Optional[dict] = None,
        **kwargs
    ):
        super().__init__(
        ...
        self.condition_pc_adapter = (
            dwm.models.adapters.ImageAdapter(**condition_pc_adapter_config)
            if condition_pc_adapter_config is not None else None
        )
    ...
    def forward(
        ...
        **kwargs
        ...
        # ------------pc----------------\
        depth = kwargs.get("condition_depth_tensor", None)
        clr = kwargs.get("condition_clr_tensor", None)
        pc_tensor = torch.cat([clr, depth*0.5], dim=-3)
        if depth is not None and clr is not None and self.condition_pc_adapter is not None:
            pc_res = self.condition_pc_adapter(pc_tensor)
            condition_residuals = _merge_res(condition_residuals, pc_res, 1.0)
        # ------------pc----------------
